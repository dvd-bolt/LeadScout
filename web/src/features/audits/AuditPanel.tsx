...
export function AuditPanel({ accountId, focus = false }: { accountId?: number; focus?: boolean }) {
  const client = useQueryClient();
  const [operationId, setOperationId] = useState<string | null>(null);
  const [notice, setNotice] = useState<Notice>(null);
  const audits = useQuery({ queryKey: ["audits", accountId ?? "none"], queryFn: () => api.audits(accountId) });
  const activeAudit = useMutation({
    mutationFn: () => api.createAudit({ account_id: accountId }),
    onSuccess: (data) => { setNotice(null); setOperationId(data.operation_id); },
  ...
  const handleTerminal = (operation: Operation) => {
    if (operationSucceeded(operation)) {
      client.invalidateQueries({ queryKey: ["audits", accountId ?? "none"] });
    }
  };

  return <section className={`${styles.card} ${focus ? styles.highlighted : ""}`} id="audits">
    <div className={styles.cardHeader}>
      <div><h2>ИИ-аудит</h2><p className={styles.meta}>Можно проверить активное резюме или отдельный текст/PDF.</p></div>
      {accountId ? <Button disabled={activeAudit.isPending} onClick={() => activeAudit.mutate()}>Проверить активное</Button> : null}
    </div>
    ...
    <IndependentAudit accountId={accountId} onOperation={setOperationId} onError={(text) => setNotice({ text, error: true })} />
    <AuditList audits={audits.data ?? []} scopeKey={accountId ?? "independent-audit"} />
  </section>;
}

function IndependentAudit({ accountId, onOperation, onError }: { accountId?: number; onOperation: (id: string) => void; onError: (text: string) => void }) {
  ...
  const busy = textAudit.isPending || pdfAudit.isPending;
...
function VacancyMatchForm({ auditId, scopeKey }: { auditId: number; scopeKey: string | number }) {
  ...
  return <div className={styles.form}>
    ...
    <form className={styles.form} onSubmit={(event) => {
      ...
      <Button className={styles.secondary} disabled={!isValid || match.isPending} type="submit">Сравнить</Button>
    </form>
  ...
  return <div className={styles.auditList}>
    {audits.length > 0 ? audits.map((audit) => <article className={styles.audit} key={audit.id}>
      <div className={styles.row}>
        <div><div className={styles.resumeTitle}>{audit.profession_name}</div><div className={styles.meta}>{formatDate(audit.created_at)}</div></div>
      ...
      <details><summary>Подробный отчёт</summary>
        <h3>Оценки</h3>
        <div className={styles.meta}>{Object.entries(audit.category_scores ?? {}).map(([name, score]) => <div key={name}>{name}: {score}/100</div>)}</div>
        <h3>Штрафы</h3>
