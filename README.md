...
- Мастер резюме хранит версионный черновик, умеет заполнять его вручную или из PDF, сравнивает
  личные данные с профилем hh.ru и продолжает частично завершённую публикацию по сохранённому ID.
  Последние правки резервируются в браузере до подтверждённого сохранения на сервере.
...
- SQLite v12 хранит роли, технические задания, административные команды, версионные черновики,
  точные ID вакансий, время ручного решения и диагностические попытки отклика.
...
Пользовательская история событий хранится 180 дней, завершённые операции — 30 дней, аудиты —
365 дней. Минимальные ID подтверждённых откликов сохраняются без текста вакансии и письма для
защиты от повторной отправки. На экране истории доступны экспорт JSON и удаление подробных данных.
