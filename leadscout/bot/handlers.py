...
def create_handlers_router(owner_id: int | None = None, *, owner_ids: tuple[int, ...] = ()) -> Router:
    ...
    async def help_message(message: Message, app_context: AppContext) -> None:
        text = (
            "ℹ️ <b>Статусы вакансий в LeadScout:</b>\n\n"
            "🟢 <b>Отклик отправлен</b> — бот успешно отправил резюме на новую вакансию на hh.ru (+1 к откликам за сегодня).\n\n"
            "⚪ <b>Откликались ранее</b> — на hh.ru уже есть ваш прошлый отклик. Бот не дублирует его и не тратит лимит.\n\n"
            "🟡 <b>Вакансия пропущена</b> — нейросеть проверила вакансию и отклонила её (не подходит под резюме или сработало стоп-слово).\n\n"
            "🔵 <b>Нужны ответы</b> — у работодателя сложная анкета/тест. Она ждёт вашей проверки на вкладке «Ждут проверки» в приложении.\n\n"
            "🔴 <b>Ошибка отклика</b> — вакансия закрыта работодателем или потребовался ввод капчи.\n\n"
            "Откройте приложение для управления поиском и откликами:"
        )
        await message.answer(
            text,
            parse_mode="HTML",
