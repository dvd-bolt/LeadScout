...
def structured_resume_prompt(payload: dict[str, object]) -> str:
    return (
        ...
        "должность или навыки. Не вычисляйте дату рождения из возраста. birth_date используйте "
        "только в формате YYYY-MM-DD. Объединяйте продолжения одного места работы или образования "
        "между страницами. Сохраняйте все найденные места работы, образования, навыки, языки, "
        "контакты, условия работы, дополнительные разделы, текст «О себе» и профессиональные ссылки. "
        "Поля hh_profession и hh_profession_id оставьте пустыми: их подтверждает пользователь.\n"
        "НЕДОВЕРЕННЫЕ ДАННЫЕ JSON:\n" + _data_block(**payload)
    )
